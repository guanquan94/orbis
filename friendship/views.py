...
def all_users(request, template_name="friendship/user_actions.html"):
    users_raw = user_model.objects.all()
    users = []
    for user in users_raw:
        if user == request.user:
            continue
        if Friend.objects.filter(to_user=user, from_user=request.user).exists():
            continue
        if Friend.objects.filter(to_user=request.user, from_user=user).exists():
            continue
